...
function unlike(liked_element, parent) {

    if (liked_element.liked) {

        liked_element.liked = false;
        parent.src = '/img/assets/unlike.svg'
        data[liked_element.id].liked = false
        change_local_storage('data', data);
        //i to show the exact page (page 1 or 2)
        // show_cards( 1 , like_data)
    }
...
function show_cards(index, array) {
    ...
    for (let i = (index * 12) - 12; i < index * 12; i++) {

        if (array.length > i && array[i].liked) {
            CARDS_CONTAINER.innerHTML += `
            <div class="card">
...
function filter_cards(rarety) {
    let filtred_array = like_data.filter((item) => item.rarety == rarety);
...
function handle_pagination(arr, activ_button_index) {
    ...
    let NUMBER_PAGINATION = Math.ceil(arr.length / 12);
...
let like_data = data.filter((item) => item.liked)
